...
| 2026-08-09 | `git status` 결과에서 `modified`와 `Untracked files`가 뭐가 다른지 헷갈림 | `modified`는 Git이 이미 추적 중인 파일이 바뀐 것, `Untracked`는 Git이 한 번도 본 적 없는 새 파일 | 둘 다 `git add`로 스테이징. 이 과정에서 커밋은 "의미 단위"로 쪼개야 한다는 것도 익힘 |
